-- Fix: login mobile-number matching only worked one direction (input without
-- +91 against a stored number with +91), not the reverse. This caused
-- Google's reviewer login ("+919000000000") to fail against a stored
-- number of "9000000000" and triggered a real rejection.
--
-- Fix: always compare on the last 10 digits of both sides, regardless of
-- whether either one carries a country code. Safe to re-run.
